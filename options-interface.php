		...
		switch ( $value['type'] ) {
			...
			case "radio":
				...
				foreach ($value['options'] as $key => $option) {
					...
					if ( isset($value['data']) ){
						$opt = preg_replace('/[^a-zA-Z0-9._\-]/', '', strtolower($key));
						$data_attr = 'data-attr="' . esc_attr( $value['data'][$opt] ) . '"';
						}
					$id = $option_name . '-' . $value['id'] .'-'. $key;
					$output .= '<input class="of-input of-radio" type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="'. esc_attr( $key ) . '" '. checked( $val, $key, false) .' ' . $data_attr . ' /><label for="' . esc_attr( $id ) . '">' . esc_html( $option ) . '</label>';
				}
			break;
			...
			case "checkbox":
					$data_attr = '';
					if ( isset($value['data']) ){
						$data_attr = ' data-attr="' . esc_attr( $value['data'] ) . '"';
						}
				$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="checkbox of-input" type="checkbox" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" '. checked( $val, 1, false) .' ' . $data_attr . ' />';
			...
			case "multicheck":
				foreach ($value['options'] as $key => $option) {
					...
					$data_attr = '';
					if ( isset($value['data']) ){
						$data_attr = ' data-attr="' . esc_attr( $value['data'][$option] ) . '"';
						}
					...
					$output .= '<input id="' . esc_attr( $id ) . '" class="checkbox of-input" type="checkbox" name="' . esc_attr( $name ) . '" ' . $checked . ' ' . $data_attr . ' /><label for="' . esc_attr( $id ) . '">' . esc_html( $label ) . '</label>';
				}
			break;
